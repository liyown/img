...
仓库根目录 [SKILL.md](https://github.com/liyown/img/blob/main/skills/img-uploader/SKILL.md) 提供 Agent 使用说明；[GitHub Action](https://github.com/liyown/img/blob/main/action.yml) 提供工作流集成。此 Action 的安装步骤仍获取已发布的 CLI，因此当前可能使用旧 Go 版本。若要在 CI 使用 Rust 0.3.0，请先按安装指南从已审核提交构建 CLI，再直接调用 img rewrite；不要把固定 Action 提交等同于固定 CLI 版本。
