...
本文说明主分支新增功能。稳定版安装包的功能以对应 Release notes 为准；完整验收状态见 [功能验收](https://github.com/liyown/img/blob/main/desktop/features-qa.md)。

## 从其他工具迁入

在桌面「设置 → 存储源」选择「导入 PicGo / PicList 配置」，打开 JSON。先查看候选存储源与无法导入的项目，再逐项「检查并添加」。已有同名配置不会被覆盖，新配置自动使用后缀名称。保存后可以测试连接，再设为默认。
...
CLI、编辑器服务、截图、文章转存和 Agent 上传都会写入本机上传收件箱。桌面运行时将它们同步进图库，显示来源；搜索也支持来源。每条记录独立发布，多个进程同时上传不会相互覆盖。桌面保存成功后才清理对应收件箱文件。
...
最长边保持比例，不放大图片。JPEG 透明部分合成白色；WebP 使用无损编码，质量参数只控制 JPEG。水印宽高限制在图片的四分之一内，默认透明度 60%。桌面水印路径必须为绝对路径。
...
监听等待大小与修改时间连续两次不变后上传；只有成功才将该版本标为已处理。--new-only 跳过启动时已有文件。桌面在「文章与目录」启动或停止监听，关闭窗口继续，退出应用停止。监听不跨重启自动恢复。
...
默认不包含图片缓存和明文凭据。没有缓存的记录仍保留链接，预览或重新上传需要重新选择原文件。钥匙串凭据及明文密钥只有明确选择后才导出，**凭据备份不加密**，请存放在私人位置。

恢复替换备份包含的设置与记录，按文件恢复缓存，不清空无关文件。恢复前生成 img-before-restore-UUID 副本。应用中的恢复按钮先安全保存队列并退出，再恢复并重新打开。损坏备份或中途失败不报告成功，失败时尝试回滚并给出恢复副本位置。原始图片与远端存储不受影响。

## 远端文件

「远端文件」打开当前存储源根目录，进入子目录、分页、复制链接，并确认删除单个文件。支持 S3 兼容服务、GitHub、WebDAV。自定义 HTTP 上传接口没有统一的浏览和删除协议。
...
删除需要列表返回的 ETag 或 GitHub SHA。文件变化时需刷新再确认；没有版本标识时不提供删除。目录删除不开放。本地清理只清理记录与缓存，**远端删除影响已有链接**。S3 启用版本控制时可能生成删除标记，旧版本仍由存储服务管理。
