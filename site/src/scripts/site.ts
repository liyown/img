...
document.querySelectorAll<HTMLPreElement>('pre.astro-code').forEach((pre) => {
  ...
  button.textContent = label('复制', 'Copy');
  ...
  button.addEventListener('click', async () => {
    try {
      ...
      button.textContent = label('已复制', 'Copied');
    } catch {
      button.textContent = label('请手动选择复制', 'Select text to copy');
    }
    window.setTimeout(() => {
      button.textContent = label('复制', 'Copy');
    }, 2000);
...
if (gaId)
  import('./analytics').then(({ initAnalytics }) => initAnalytics(gaId));
